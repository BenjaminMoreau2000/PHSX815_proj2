...
if(y=="Y"):
    ...
    print("input lower limit of the dices uniform weight (it may cause errors for values above 2 or so):")
    Dice_Power_Lower=float(input())
    print("input upper limit of the dices uniform weight (it may cause errors for values above 2 or so):")
...
###
    p1=[100000000]#make the p1 vector, starting with an aritrary value, and in the end it will represent probabilitty
    ...
    for mm in range(Num_sides):
        p1[mm]=p1[mm]/Sp1
    #Those probabilities for the sides of the loaded die sum to 1
    ...
    for l in range(Num_rolls):
        ...
        res_f0.write(str(a)+" "+str(Dice_power)+"\n")

    log0.write(str(LLR)+" " +str(Dice_power)+"\n")
...
###
    ...
    log1.write(str(LLR)+" "+str(Dice_power)+"\n")
